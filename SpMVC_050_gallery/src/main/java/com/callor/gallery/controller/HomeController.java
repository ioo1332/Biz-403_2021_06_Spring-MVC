...
public class HomeController {
	...
	// location.href=${rootPath}/dumy/gallery/detail로 JS에서 실행했을때
	// 처리할 method
	 ...
	 * 전송 button을 클릭하면 이 method가 request를 수신하고 input box에 입력한 문자열을
	 * str 변수에 담기게 된다
	 */
	@RequestMapping(value="/dumy/gallery/detail/{id}",method=RequestMethod.POST)
	public String dumy(String str) {
		return "home";
	}
	
	// localhost:8080/rootPath/dumy/gallery/detail/image 요청했을때
	// request를 처리할 method
	 ...
	 * 이 클래스는 @requsetParam을 붙이면 400 오류가 난다
	 */
	...
	public String home(@RequestParam("one_file") MultipartFile one_file, 
		 ...
		 *  
		 *  m_file객체에서 getFiles method를 사용하여 파일들 List를 추출한다
		 *  List<MultipartFile> type의 객체에 담는다
		 *  
		 *  이때 getFiles 에는 form에서 설정된 input tag의 name값을 지정해준다
