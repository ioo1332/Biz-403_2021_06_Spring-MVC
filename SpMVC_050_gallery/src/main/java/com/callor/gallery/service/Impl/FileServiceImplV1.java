...
public class FileServiceImplV1 implements FileService{	
	...
	public String fileUp(MultipartFile file) throws Exception {
		 ...
		 * 쉽게 업로드 구현이 된다
		 * 
		 * 하지만 만약 같은이름의 파일을 중복해서 업로드하면 먼저 업로드 된 파일이
		 * 변경되는 문제 발생한다
		 * 
		 * 해커에 의해 같은 이름으로 파일들 업로드해버리면 저장된 원래 파일들이 모두 변조될수있다
		 * 이러한 문제를 방지하기위해 UUID문자열을 생성하고
		 * UUID+원래 파일 이름. 원래 확장자 형식으로 업로드를 수행한다
		 * 이런방식으로 파일을 저장하면 해킹의 위험을 다소 막을수 있다.
		 */
