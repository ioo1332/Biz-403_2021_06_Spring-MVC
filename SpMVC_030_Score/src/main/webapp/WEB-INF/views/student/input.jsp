...
<form id="student_input" method="POST">
	<fieldset>
		<div><label>학번</label>
		<input name="st_num" id="st_num" value=<c:out value="${STD.st_num }" default="0"/> placeholder="학번을 입력해주세요"/></div>
	...
	document.querySelector("form#student_input").addEventListener("click",(e)=>{
		...
		if(target.tagName==="BUTTON"){
			/* tag에 class를 지정할때 1개만 지정할수도 있지만
				...
				이때는 if(target.className==="className")형식으로 class를 검사할수 없다
				이럴때는 includes를 사용하여 포함된 클래스를 검사해야한다
			*/
			if(target.className.includes("save")){
				//여기에 event처리 코드를 작성할수있지만 
