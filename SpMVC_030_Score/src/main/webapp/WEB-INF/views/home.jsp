...
nav.fixed{
	...
	box-shadow: 3px 3px 3px rgba(0,0,0,0,0.5);
	trantition:1s;
}
nav ul{
	list-style: none;
...
nav li{
	padding: 16px 12px;
	border-bottom: 3px solid transrarnet;
	trantition:1s;
	cursor: pointer;
}
nav li:hover{
...
section#main_sec{
	...
	section#main_sec 에 overflow속성부여하기
	overflow: auto;
	*/
}
...
div.btn_box{
	width: 90%;
	/* table의 margin10px auto로 설정되어있기때문에
...
<script>
/*
 * JS코드에서 event를 등록할때 현재 화면에 없는 DOM요소에
 * addEvent를 설정하면 없는 합수라는 오류가 발생한다
 * 그 이유는 현재 화면에 없는 DOM요소를 querySelector하면 
 * 그결과값이 null이기 때문에 발생하는 문제이다
 *
 * JS코드를 통합하여 모음으로 관리 할때는
 * addEVent를 하려고하는 요소가 있는지 먼저 검사한후 addEvent를 수행해주어야한다
 */
...
document.addEventListener("scroll",()=>{
	//html문서 전체의 크기 좌표등을 추출하기
	let doc_bound=document.querySelector("HTML").getBoundingClientRect();
	let doc_top=doc_bound.top
	/* 화면이 아래방향으로 스크롤 될때 화면 문서의 top좌표를 추출하여
		header box의 높이와 비교
		header box의 높이에 -1을 곱하고 그값보다 작아지면
		=== header box가 화면에서 사라지면
		nav에 fixed라는 class를 부착하고 ==header box가 화면에서 나타나면
