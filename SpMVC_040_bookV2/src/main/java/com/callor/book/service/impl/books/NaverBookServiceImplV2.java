 ...
 * Service interface와 ServiceV1에 구현된 method코드를 모두 그대로 상속받는다
 * 
 * Service의 method queryURL(),getJsonString(),getNaverList()중에서 
 * getNaverList()method를 재 정의 하려고 한다
 * 
 * queryURL(),getJsonString() method는 그대로 사용하고
 * getNaverList()만 다시 작성하겠다
 * 
 * ServiceV1의 getNaverList()는 json-simple을 사용하여
 * JSON parsing을 수행한후 List를 return하는 코드
 * 
 * ServiceV2에서는 getNaverList()method를 다시 작성하여 gson을 사용하여 JSON parsing을 수행하는 코드로 작성
 */
/*
 ...
 * NaverBookServiceImplV1 클래스에 정의된 queryURL(),getNaverList()method를 물려 받았다
 * 
