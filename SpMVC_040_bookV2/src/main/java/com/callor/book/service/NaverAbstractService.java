...
/*
 * abstract class 키워드를 추가 함으로서 추상클래스가 된다
 * 추상클래스
 ...
 * 1. 상속되는 부모 클래스의 역할
 * 		스스로는 객체로 생성될수 없다
 * 2. 추상 메서드를 포함하는데 상속받은 곳에서는 반드시 추상메서드를 구현해야한다
 * 3. 상속 받은 클래스들에 필요한 공통 처리 method는 일반 클래스처럼 자체적으로 구현코드를 가지고있다
 * 4. 공통처리 method도 필요에 따라 상속받는곳에서 재 정의 할수있다
 * 5. 인터페이스의 역할을 수행
